...
const standardizePhoneNumbers = (arr) => {
 return arr.map(string => (string.substring(1,4) + string.substring(6,9) + string.substring(10,15)));
};
...
const onlyOddChars = (str) => {
    return str.split('').filter((char, i) => i % 2 === 1).join('');
};
...
const findEvery = (arr, target) => {
    return arr.filter((string) => string.includes(target)).length === arr.length;
...
const unenrollBrook = (arr) => {
  return arr.map(array=> array.filter(string => !string.includes('Brook')));
};
